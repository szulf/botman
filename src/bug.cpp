...
void bug_move(float dt, BugData& bug_data, const RobotData& robot_data, const MapData& map_data) {
    if (bug_data.state == BUG_DEAD && GetTime() - bug_data.dead_time < 1) {
    ...
    if (bug_data.state == BUG_DEAD) {
        bug_data.death_display = false;
        bug_data.tint.a = 80;
    }

    if (bug_data.state == BUG_DEAD && in_about_center(bug_data.pos, map_data) && get_grid_from_pos(bug_data.pos, map_data) == map_data.spawner_pos) {
    ...
    if (bug_data.state == BUG_RESPAWNING && GetTime() - bug_data.dead_time > 1) {
        bug_data.state = BUG_ALIVE;
        return;
    }

    if (bug_data.state == BUG_RESPAWNING) {
        return;
    }
    ...
    if (
            (get_tile(next_grid_pos, map_data) == TILE_WALL || (get_tile(next_grid_pos, map_data) == TILE_SPAWNER && next_grid_pos != bug_data.path.back())) &&
            CheckCollisionRecs(
                {
        ...
        if (get_tile(grid_pos, map_data) == TILE_PORTAL) {
